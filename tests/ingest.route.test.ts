...
describe("POST /api/ingest — valid submissions and storage integrity", () => {
  ...
  it("returns 201 (not 500) on a retried duplicate deviceId+timestamp POST, storing exactly one row (D-33/D-34)", async () => {
    ...
    const payload = {
      deviceId: DEVICE_ID,
      timestamp,
      vitals: { heartRate: 140, spo2: 97, temperature: 37.1, activityScore: 2 },
    };

    const firstRes = await POST(makeRequest(payload, { "x-api-key": VALID_KEY }));
    expect(firstRes.status).toBe(201);
    const firstJson = await firstRes.json();
    expect(firstJson).toEqual({ status: "ok" });

    const secondRes = await POST(makeRequest(payload, { "x-api-key": VALID_KEY }));
    expect(secondRes.status).toBe(201);
    const secondJson = await secondRes.json();
    expect(secondJson).toEqual({ status: "ok" });
